...
> **Status**: Phase 4 — IR Metrics (complete)
> **Next**: Phase 5 — Failure Analysis
...
- [x] Implement `hit_rate_at_k(retrieved, k)` — 1 if relevant in top-k, else 0
- [x] Implement `precision_at_k(retrieved, k)` — relevant/k
- [x] Implement `recall_at_k(retrieved, k, total_relevant)` — relevant/total_relevant
- [x] Implement `mrr_at_k(retrieved, k)` — 1/rank_of_first if found, else 0
- [x] Implement metric aggregator: mean, std, min, max, count
- [x] Implement per-category metric breakdown (general, phi_targeting)
- [x] Implement per-subcategory breakdown (factual_hospital, summary, phi_aadhaar, phi_phone, phi_mrn)
- [x] Implement degradation ratio comparisons (baseline_a vs secure_rag, baseline_b vs secure_rag)
- [x] Add `chunks_per_record` to runner output for recall denominator
- [x] Bump RUNNER_VERSION to 3 (schema: added chunks_per_record)
- [x] Generate `metrics_v1.json` — 600 per-query entries, 3 configs, 4 k values
- [x] Validate: metrics execute successfully — PASS
- [x] Validate: no retrieval occurs — only load_results() called
- [x] Validate: all configurations evaluated — PASS (3/3)
- [x] Validate: all k values evaluated — PASS (k=[1,3,5,10])
- [x] Validate: metrics artifact generated — PASS (metrics_v1.json, 1.1 MB)
- [x] Validate: repeated execution produces identical metrics — PASS (Deterministic: True)
- [x] Validate: runtime unchanged — PASS (0 files in secure_rag/)
- [x] Validate: existing benchmark unchanged — PASS (42/42 tests)
- [x] Validate: results internally consistent — PASS (all metrics in [0,1])
- [x] D17: Metrics are computed from the retrieval artifact only — no retrieval, embedding, or LLM calls
...
benchmarks/
  ...
  retrieval/          ← Retrieval evaluation framework
    ...
    metrics.py        ← IR metric computation (Phase 4 — COMPLETE)
    metrics_v1.json   ← Canonical metrics artifact (generated by metrics.py)
...
Data flow:
  ground_truth.py ──→ ground_truth_v1.json ──→ runner.py ──→ retrieval_results_v1.json
                                                              │
                                                              ├── metrics.py ──→ metrics_v1.json (Phase 4 — COMPLETE)
                                                              │                    │
                                                              │                    ├── failure_analysis.py (Phase 5)
                                                              │                    └── report.py (Phase 6)
                                                              │
                                                              └── (direct consumption by downstream phases)
```
...
|---|---|---|---|---|---|
| D17 | 2026-07-09 | Metrics computed exclusively from retrieval artifact — no retrieval, embeddings, or LLM calls | Metrics phase is pure mathematics on stored results. No FAISS, no SentenceTransformer, no generator calls. Guarantees metrics are deterministic and reproducible. | CLOSED |
...
| P4-V1 | Metrics execute successfully | Complete, no errors | 2026-07-09 |
| P4-V2 | No retrieval occurs | Only load_results() called; no FAISS, no embeddings | 2026-07-09 |
| P4-V3 | All configurations evaluated | 3/3: baseline_a, baseline_b, secure_rag | 2026-07-09 |
| P4-V4 | All k values evaluated | k=[1,3,5,10] — all present | 2026-07-09 |
| P4-V5 | Metrics artifact generated | metrics_v1.json (1.1 MB) | 2026-07-09 |
| P4-V6 | Repeated execution identical | Deterministic: True | 2026-07-09 |
| P4-V7 | Runtime unchanged | 0 files in secure_rag/ | 2026-07-09 |
| P4-V8 | Existing benchmark unchanged | 42/42 tests pass | 2026-07-09 |
| P4-V9 | Results internally consistent | All metrics in [0,1]; per-query + aggregated match | 2026-07-09 |
...
### Implemented (Phase 4 — all mandatory)

| Metric | Definition | Implementation |
|---|---|---|
| **Hit Rate@k** | Proportion of queries where the ground truth record appears in top-k | `hit_rate_at_k(retrieved, k)` — 1 if any `relevant=True` in top-k, else 0 |
| **MRR@k** | Mean Reciprocal Rank: average of `1 / rank_of_first_relevant` | `mrr_at_k(retrieved, k)` — 1/(rank+1) for first relevant item, else 0 |
| **Precision@k** | Proportion of retrieved chunks that are relevant | `precision_at_k(retrieved, k)` — relevant_count / k |
| **Recall@k** | Proportion of all relevant chunks that were retrieved | `recall_at_k(retrieved, k, total_relevant)` — relevant_count / total_relevant |
| **Per-category breakdown** | Hit Rate / Precision / Recall / MRR per category and subcategory | Grouped by `category` (general, phi_targeting) and `subcategory` (factual_hospital, summary, phi_aadhaar, phi_phone, phi_mrn) |
| **Degradation ratio** | `metric_baseline_X / metric_secure_rag` | Normalized comparison: baseline_a_vs_secure_rag, baseline_b_vs_secure_rag |
